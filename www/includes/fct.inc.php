...
{//return si (oui ou non)ya qqchose dans la colone idvesiteur du tableau session
...
function connecterComptable($idC,$nom,$prenom){
    $_SESSION['idComptable'] = $idC;
    $_SESSION['nom'] = $nom;
    $_SESSION['prenom'] = $prenom;
}
/**C
...
function moisPrecedant($mois){
    //prend en param un moi et retourne le precedant
...
function moisSuivant($mois){
    //prend en param un moi et retourne le suivant
    $numMois = substr($mois, 4, 2);
    $numAnnee = substr($mois, 0, 4);
    if($numMois==12){
            $numMois=01;
...
function getLesMois($mois){
    ...
    $lesDouzeMois = array();//on cre un tableau ou on va inserer les 12 dernier mois
    for($i = 0; $i <=12; $i++){
    $mois= moisPrecedant($mois);
    $numMois = substr($mois, 4, 2);
    $numAnnee = substr($mois, 0, 4);
    $lesDouzeMois[] = array(
            'mois' => $mois,
