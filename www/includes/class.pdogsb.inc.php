...
//    private static $monPdoGsb = null; //on initialise deux variable
    ...
    //constructeur initialiser tout les variable qui ont a pas ete initialiser la seul c'est $monPdo
        PdoGsb::$monPdo = new PDO(//class PDO universelle initialise pdo
     ...
     */
    public function __destruct() {//detruit PDO ramassse miette liberer la place pour le prochain detruit la connexion a la bdd 
        PdoGsb::$monPdo = null;
    }
     ...
     */
    public static function getPdoGsb() {//cxre l unique objet de la class qui est $monPdoGsb si il est null
     ...
     */
//    àparam les parametere c tre important savoir ce qu elle fait o
    public function getInfosVisiteur($login, $mdp) {
        $requetePrepare = PdoGsb::$monPdo->prepare(
                'SELECT visiteur.id AS idV, visiteur.nom AS nomV, '
                . 'visiteur.prenom AS prenomV '
                . 'FROM visiteur '
    ...
    public function getInfosComptable($login, $mdp) {
        $requetePrepare = PdoGsb::$monPdo->prepare(
                'SELECT comptable.id AS idC, comptable.nom AS nomC, '
                . 'comptable.prenom AS prenomC '
                . 'FROM comptable '
    ...
    public function getLesVisiteur() {
        ...
        $requetePrepare->execute();
        $lesVisiteur = $requetePrepare->fetchAll();
        return $lesVisiteur;
    }

    public function getnbjustificatif($idVisiteur, $leMois) {
        $requetePrepare = PdoGSB::$monPdo->prepare(
                'SELECT nbjustificatifs FROM fichefrais '
 ...
 public function mFraisHorsforfait($mois,$idVisiteur,$date,$libelle,$montant) {
     $dateFr = dateFrancaisVersAnglais($date);
        ...
        'UPDATE lignefraishorsforfait '
            . 'SET  lignefraishorsforfait.date = :unDate,  lignefraishorsforfait.libelle= :unLibelle, lignefraishorsforfait.montant = :unMontant'
            . 'WHERE lignefraishorsforfait.idVisiteur=:unVisiteur '
            . 'AND lignefraishorsforfait.mois = :unMois'
        );
        
        $requetePrepare->bindParam(':unDate', $dateFr, PDO::PARAM_STR);
        $requetePrepare->bindParam(':unLibelle', $libelle, PDO::PARAM_STR);
        $requetePrepare->bindParam(':unMontant', $montant, PDO::PARAM_INT);
 ...
 public function  majFraisHorsForfait($id,$libelle,$date,$montant){
    ...
    public function  getvisiteuretmois($id){
     ...
     */
    public function majFicheFrais($idVisiteur,$leMois,$montanttotal){
        $requetePrepare = PdoGSB::$monPdo->prepare(
                'UPDATE fichefrais '
        ...
        $requetePrepare->bindParam(':total',$montanttotal, PDO::PARAM_STR);
        $requetePrepare->bindParam(':unIdVisiteur', $idVisiteur, PDO::PARAM_STR);
        $requetePrepare->bindParam(':unMois', $leMois, PDO::PARAM_STR);
        $requetePrepare->execute();
    ...
    public function montantfraisforfait($idVisiteur ,$leMois) {
         $requetePrepare = PdoGSB::$monPdo->prepare(
                ...
                .'WHERE lignefraisforfait.idvisiteur = :unIdVisiteur '
                .'AND lignefraisforfait.mois = :unMois '
                );//faut multiplier la quantite de ligne frais forfait par le montant de frais forfait
        $requetePrepare->bindParam(':unIdVisiteur', $idVisiteur, PDO::PARAM_STR);
        $requetePrepare->bindParam(':unMois', $leMois, PDO::PARAM_STR);
        $requetePrepare->execute(); 
        return $requetePrepare->fetch();
        ...
        public function  getlesvisiteuretmois(){
        $requetePrepare = PdoGSB::$monPdo->prepare(
